...
import { getFirst, getUserAccountScope, parseJson, parseQuery, withErrorHandling } from "../platform/helpers.js";
import { beginIdempotentRequest, storeIdempotencyResponse, storeIdempotentJsonResponse } from "../platform/idempotency.js";
...
export const createOrderRoutes = (registry: MarketRegistry) => {
  ...
  router.post(
    ...
    withErrorHandling(async (c) => {
      ...
      const idempotency = await beginIdempotentRequest(c, userId, parsed.data);
  ...
  router.get(
    ...
    withErrorHandling(async (c) => {
      ...
      if (userId !== "admin") {
        ...
        predicates.push(eq(orders.accountId, accountScope.account.id));
  ...
  router.get(
    ...
    withErrorHandling(async (c) => {
      ...
      if (userId !== "admin") {
        ...
        if (!accountScope.account || accountScope.account.id !== order.accountId) {
  ...
  router.delete(
    ...
    withErrorHandling(async (c) => {
      ...
      if (cancellation.kind !== "cancelled") {
        ...
        if (idempotency.candidate) {
          await storeIdempotencyResponse(idempotency.candidate, 200, payload);
        }
